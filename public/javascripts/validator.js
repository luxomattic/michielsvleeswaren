...
(function($) {
	...
	$.formValidator.validate = function(obj,opts) {
		...
		if (obj.hasClass('req-same')) {
			...
			if (!tmpresult) {
				...
				$(opts.scope+' .req-same[rel="'+group+'"]').removeClass('error-same');
			}
		...
		if (obj.hasClass('req-both')) {
			...
			if (valAttr != '') {
				...
				if (!tmpresult) {
					$(opts.scope+' .req-both[rel="'+group+'"]').addClass('error-both');
					errorTxt = opts.errorMsg.reqBoth;
				} else {
					$(opts.scope+' .req-both[rel="'+group+'"]').removeClass('error-both');
				}
			}
	...
	$.formValidator.defaults = {
		...
		errorDiv		:	'#warn',
		errorMsg		: 	{
								reqString		:	'Je hebt nog niet alle velden ingevuld.',
								reqDate			:	'Je hebt een foutieve datum opgegeven. (dd-mm-yyyy)',
								reqNum			:	'Je hebt tekst ingevuld in numerieke velden.',
								reqMailNotValid	:	'Je e-mailadres is niet juist gevormd.',
								reqMailEmpty	:	'Je hebt nog geen e-mailadres ingevuld.',
								reqSame			:	'Je hebt niet 2 keer hetzelfde paswoord ingevuld.',
								reqBoth			:	'De verplichte velden zijn niet ingevuld.',
								reqMin			:	'Je moet minimum %1 karakters invoeren.'
							},
		customErrMsg	:	'',
		extraBoolMsg	:	'',
		dateSeperator	:	'-',
